...
let processArray = (t, input, expect) => {
  Array.from(input)
    .forEach((str, index) => t.is(util.beautify(str), expect[index]));
}
...
test("should return strings camel cased if it contains two explicit words",
  processArray,
  ["one", "one-two"],
  ["one", "oneTwo"]
);

test("should return camel cased string redacting unwanted symbols",
  processArray,
